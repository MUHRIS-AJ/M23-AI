...
// Shaders are constant — hoisted to module scope so they keep a stable identity
// across renders (otherwise the main effect's deps change every render and the
// WebGL context is torn down + rebuilt continuously).
const VERT = /* glsl */ `
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => {
    ...
    // ogl's gl is a WebGL2 context augmented with { renderer, canvas }; derive
    // the exact type from Renderer so Triangle/Program/Mesh accept it.
    ...
    try {
      ...
      // ogl types the canvas as HTMLCanvasElement | OffscreenCanvas; in the browser
      // it's always a real <canvas>, so narrow once for DOM/style operations.
